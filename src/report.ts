...
function severity(r: PageResult): number {
  if (!r.a.ok || !r.b.ok) return 3;
  if (r.diff.kind === "layout") return 2;
  if (r.diff.kind === "diff") return 1;
  return 0;
}
...
export async function writeReport(report: Report, file: string): Promise<void> {
  ...
  const counts = {
    ...
    match: rows.filter((r) => r.diff.kind === "match").length,
  ...
  const data = rows.map((r) => ({
    ...
    kind: !r.a.ok || !r.b.ok ? "error" : r.diff.kind,
...
<style>
  ...
  .skipped { background:#555; color:#fff; }
...
<table id="t">
  <thead><tr><th data-k="kind">status</th><th data-k="path">path</th><th data-k="viewport">viewport</th><th data-k="pct">diff %</th><th data-k="status">HTTP a/b</th><th data-k="error">notes</th></tr></thead>
  <tbody></tbody>
</table>
